...
TEST(StateMachineTests, InitiateStateKeepsDoubleOnTwoCards) {
  const State state = InitiateState(RuleSet(), Turn::Player, PlayerHand(10, false, 2),
                                    DealerHand(6, false, 1),
...
TEST(StateMachineTests, InitiateStateRemovesDoubleWhenNotTwoCards) {
  const State state = InitiateState(RuleSet(), Turn::Player, PlayerHand(15, false, 3),
                                    DealerHand(6, false, 1),
                                    Action::Hit | Action::Stand | Action::Double);
...
TEST(StateMachineTests, InitiateStateDealerEntersForcedHit) {
  const State state = InitiateState(RuleSet(), Turn::Dealer, PlayerHand(18, false, 2),
                                    DealerHand(9, false, 1),
                                    Action::Hit | Action::Stand | Action::Double);
...
TEST(StateMachineTests, StandMovesToDealer) {
  const State next = Stand(RuleSet(), PlayerTurn(PlayerHand(18, false, 2), DealerHand(9, false, 1)));
  EXPECT_EQ(next.turn, Turn::Dealer);
  EXPECT_EQ(next.playerHand.score, 18);
}

TEST(StateMachineTests, HitStaysWithPlayerWhenNoBust) {
  const State next = Hit(RuleSet(), PlayerTurn(PlayerHand(12, false, 2), DealerHand(9, false, 1)), Card(5));
...
TEST(StateMachineTests, HitBustEndsRound) {
  const State next = Hit(RuleSet(), PlayerTurn(PlayerHand(16, false, 2), DealerHand(9, false, 1)), Card(10));
...
TEST(StateMachineTests, HitAddsToDealerOnDealerTurn) {
  const State next = Hit(RuleSet(), DealerTurn(PlayerHand(18, false, 2), DealerHand(10, false, 1)), Card(5));
...
TEST(StateMachineTests, StandEndsRoundOnDealerTurn) {
  const State next = Stand(RuleSet(), DealerTurn(PlayerHand(18, false, 2), DealerHand(19, false, 2)));
  EXPECT_EQ(next.turn, Turn::End);
  EXPECT_TRUE(IsTerminal(next));
}
...
TEST(StateMachineTests, ResultBothBlackjacksPush) {
  const State state(Turn::End, PlayerHand(21, false, 2), DealerHand(21, false, 2));
  EXPECT_EQ(Result(state), Outcome::Push);
}

TEST(StateMachineTests, ResultPlayerBlackjackBeatsNonBlackjack) {
  const State state(Turn::End, PlayerHand(21, false, 2), DealerHand(21, false, 3));
  EXPECT_EQ(Result(state), Outcome::PlayerWins);
}
...
TEST(StateMachineTests, InitialPlayerStateAllowsAllActions) {
  const State state = PlayerTurn(PlayerHand(10, false, 2), DealerHand(6, false, 1));
  EXPECT_TRUE(IsAllowed(state.allowedActions, Action::Hit));
  EXPECT_TRUE(IsAllowed(state.allowedActions, Action::Stand));
  EXPECT_TRUE(IsAllowed(state.allowedActions, Action::Double));
}
...
TEST(StateMachineTests, HitRemovesDoubleFromAllowedActions) {
  const State next = Hit(RuleSet(), PlayerTurn(PlayerHand(12, false, 2), DealerHand(6, false, 1)), Card(3));
...
TEST(StateMachineTests, BustLeavesNoAllowedActions) {
  const State next = Hit(RuleSet(), PlayerTurn(PlayerHand(16, false, 2), DealerHand(6, false, 1)), Card(10));
...
TEST(StateMachineTests, StandEndsRoundWhenDealerMustStand) {
  const State next = Stand(RuleSet(), PlayerTurn(PlayerHand(18, false, 2), DealerHand(19, false, 2)));
  EXPECT_EQ(next.turn, Turn::End);
  EXPECT_TRUE(IsTerminal(next));
  EXPECT_EQ(next.allowedActions, Action::None);
}
...
TEST(StateMachineTests, DealerForcedActionRespectsHitOnSoft17) {
  RuleSet ruleset;
  ruleset.hitOnSoft17 = true;
  const State next = Stand(ruleset, PlayerTurn(PlayerHand(18, false, 2), DealerHand(17, true, 2)));
  EXPECT_TRUE(IsAllowed(next.allowedActions, Action::Hit));
}

TEST(StateMachineTests, StandAfterDoubleMovesToDealer) {
  const State doubled = Double(PlayerTurn(PlayerHand(10, false, 2), DealerHand(9, false, 1)), Card(9));
